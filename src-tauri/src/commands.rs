...
pub struct FileJobRequest {
    ...
    /// the file, so nothing on the Rust side had audio to work with. Local files
    /// now arrive as a path (from the Tauri dialog or a file drop) and Rust
    /// derives the name, the size and the content hash itself.
    pub path: String,
    pub model_id: String,
    #[serde(default = "default_task")]
    pub task: TaskType,
...
/// is what lets anything on the Rust side assume there is audio on disk for ANY
/// job, rather than only for the YouTube ones.
///
...
///
/// 16 kHz, mono, signed 16-bit PCM: exactly what the webview's
/// `AudioContext({sampleRate: 16000})` decodes without resampling. Both source
/// types go through this one argument list on purpose — a second ffmpeg
/// invocation elsewhere is a second thing to drift, and a drifted sample rate is
...
/// audio for a job can rely on this.
#[tauri::command]
...
// write -- it never re-transcribes, never re-runs speaker detection, and never
// rewrites a single segment. That is not an optimization; it is the only
// design in which renaming a speaker is possible at all once the audio has
// been cleaned up (see `PREPARED_AUDIO_TTL_HOURS`), which for any transcript
// older than a day it has.
//
// Names key on the SOURCE, because speaker detection is a property of the
// AUDIO: a name given to a speaker today must still be attached when the same
// file is re-transcribed under a different model preset -- which lands on a
// different transcript row but the same source. Both commands take a JOB id
// (the only handle the frontend has) and resolve it with `source_id_for_job`.
//
// Speaker ids are stable only within ONE speaker-detection run, so re-running
// it on the same audio drops the source's names -- see
// `Store::persist_transcript`. That clearing is the reason a name can never
// silently move onto the wrong voice.
// ---------------------------------------------------------------------------
...
/// transcript in which "no speaker was detected here" and "the user pressed
...
mod tests {
    ...
    ///
    /// 16 kHz mono 16-bit PCM is the format the whole pipeline agrees on.
    /// ffmpeg will happily produce something else if these flags drift, and
    /// NOTHING downstream would throw: the webview resamples whatever it
    /// decodes, so transcription would look fine while anything reading this
    /// WAV directly gets audio at the wrong rate. That is a wrong-answer bug,
    ...
    /// WebVTT -- a cue that LOOKS unattributed. "No speaker was detected" and
